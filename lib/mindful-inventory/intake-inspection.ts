...
export type InventoryInspectionStatus = "draft" | "in_progress" | "complete" | "cancelled";
...
export type InventoryInspectionView = {
  ...
  performedByPartnerId: string | null;
};
...
export type InventoryFindingView = {
  ...
  severity: InventoryFindingSeverity | null;
...
export type InventoryIntakeInspectionData = {
  intake: InventoryIntakeView | null;
  mechanicalInspection: InventoryInspectionView | null;
...
export async function getInventoryIntakeInspectionData(
  ...
  vehicleId: string,
): Promise<InventoryIntakeInspectionData> {
  const [intakeResult, inspectionResult, findingsResult] = await Promise.all([
    ...
    supabase
      ...
      .select("id,inspection_type,status,summary,started_at,completed_at,performed_by_user_id,performed_by_partner_id")
      .eq("vehicle_id", vehicleId)
  ...
  const inspectionRow = inspectionResult.data;

  const intake: InventoryIntakeView | null = intakeRow
    ? {
        id: intakeRow.id,
        status: intakeRow.status as InventoryIntakeStatus,
        mileage: intakeRow.mileage,
        keysCount: intakeRow.keys_count,
  ...
  const mechanicalInspection: InventoryInspectionView | null = inspectionRow
    ? {
        id: inspectionRow.id,
        inspectionType: inspectionRow.inspection_type,
        status: inspectionRow.status as InventoryInspectionStatus,
        summary: inspectionRow.summary,
        startedAt: inspectionRow.started_at,
        completedAt: inspectionRow.completed_at,
        performedByUserId: inspectionRow.performed_by_user_id,
        performedByPartnerId: inspectionRow.performed_by_partner_id,
      }
    : null;

  const findings: InventoryFindingView[] = (findingsResult.data || []).map((row) => ({
    id: row.id,
    intakeId: row.intake_id,
    inspectionId: row.inspection_id,
    source: row.source as InventoryFindingSource,
    title: row.title,
    description: row.description,
    category: row.category,
    subcategory: row.subcategory,
    severity: row.severity as InventoryFindingSeverity | null,
    confidence: row.confidence,
    certainty: row.certainty,
    estimatedCostLow: toNullableNumber(row.estimated_cost_low),
  ...
  return {
    ...
    findings,
    planningReady:
      intake?.status === "complete" && mechanicalInspection?.status === "complete",
  };
}
