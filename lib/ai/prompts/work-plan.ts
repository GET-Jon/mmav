...
Rules:
- Findings supplied in VALIDATED FINDINGS have already completed mechanic review AND Owner scope approval. They are approved operational scope, not merely observations awaiting another approval decision.
- Some accepted findings also have an Owner-authorized spend ceiling. Others were routed to a different Partner with the final quote and spend approval still pending. Do not confuse scope approval with spend authorization.
...
- A finding with mechanicalValidationStatus = confirmed should be planned according to the confirmed evidence and Owner-approved scope.
...
- mechanicalCanPerform = false means the inspecting mechanic explicitly cannot perform that accepted scope. Preserve that as an assignment constraint in the rationale/assumptions: another capable partner is required. The application carries the Owner-selected Partner forward after your response. Do not imply that the inspector should perform it.
- When mechanicalCanPerform = false, the performing Partner's final quote may still be pending. Do not treat missing final pricing as a reason to reopen or decline the already-approved work scope.
...
- mechanicalProposedLaborPrice is the inspector's proposed LABOR price only. It is known labor evidence, not by itself the total job cost and not necessarily the performing Partner's quote when mechanicalCanPerform = false.
...
- Do not broaden scope or add unrelated repairs in an attempt to spend an estimated or authorized amount.
...
- For accepted Finding work items, use decision approved. The Owner has already approved that supplied repair/diagnostic scope. Spend authorization may either already exist or remain pending with the routed Partner quote.
...
- Cost source should be ai_estimate when you are estimating from supplied evidence. Use unknown when a meaningful estimate cannot responsibly be made. The application will replace cost fields with an Owner-authorized amount only when an actual spend ceiling exists.
...
- managerInvestigationRequired may still be true for an upgrade with unresolved compatibility/cost uncertainty, but should not be used to undo an already-approved Finding scope.
...
- For accepted Findings, keep the estimate consistent with supplied mechanic/part evidence. If the performing Partner's final quote is pending, say so in costSourceDetail and do not present an estimate as authorized spend.
...
- If a mechanic already supplied a labor price, preserve it as known labor evidence and estimate only missing components needed to form a useful total range, unless a different Partner will perform the work; in that case treat the final performing-Partner quote as pending.
...
- planningAmount for an ai_estimate should normally be a sensible midpoint or expected value inside estimatedCostLow–estimatedCostHigh. When the application has a real Owner-authorized ceiling it may replace this value after generation.
...
OWNER-APPROVED MECHANICAL FINDINGS
${JSON.stringify(input.findings, null, 2)}

OWNER-REQUESTED UPGRADES WITH MECHANICAL REVIEW
${JSON.stringify(input.upgrades, null, 2)}

Every supplied Finding has already been accepted by the Owner as operational scope. Some have an authorized spend ceiling; others are routed with final quote/spend approval pending. Represent every finding ID exactly once, keep accepted Finding work separate from upgrade work, preserve mechanic scope and performability, and do not add unrelated repair scope. For requested Upgrades, mechanical review remains technical feasibility evidence rather than final authorization. Keep labor time and elapsed turnaround separate using the definitions in the system instructions.`;
}
