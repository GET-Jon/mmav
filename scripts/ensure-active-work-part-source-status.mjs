...
const sourceHelperAnchor = `  function sourceLabel(part: InventoryPartView) {
    const source = sourceFor(part);
    if (!source) return null;
    return sourceChoices.find((item) => item.value === source)?.label || labelize(source);
  }`;

const sourceHelpers = `${sourceHelperAnchor}
...
replaceIfPresent(
  `{existing ? <span className={\`rounded-full px-2 py-0.5 text-[9px] font-black \${existing.status === "backordered" ? "bg-amber-50 text-amber-800" : existing.status === "received" || existing.status === "installed" ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-700"}\`}>{existing.status === "backordered" ? "Delayed" : labelize(existing.status)}</span> : null}`,
  `{existing && fulfillmentLabel(existing) ? <span className={\`rounded-full px-2 py-0.5 text-[9px] font-black \${fulfillmentClass(existing)}\`}>{fulfillmentLabel(existing)}</span> : null}{existing && lifecycleLabel(existing) ? <span className={\`rounded-full px-2 py-0.5 text-[9px] font-black \${lifecycleClass(existing)}\`}>{lifecycleLabel(existing)}</span> : null}`,
);
...
replaceIfPresent(
  `<span className={\`rounded-full px-2 py-0.5 text-[9px] font-black \${part.status === "backordered" ? "bg-amber-50 text-amber-800" : part.status === "received" || part.status === "installed" ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-700"}\`}>{part.status === "backordered" ? "Delayed" : labelize(part.status)}</span>`,
  `{fulfillmentLabel(part) ? <span className={\`rounded-full px-2 py-0.5 text-[9px] font-black \${fulfillmentClass(part)}\`}>{fulfillmentLabel(part)}</span> : null}{lifecycleLabel(part) ? <span className={\`rounded-full px-2 py-0.5 text-[9px] font-black \${lifecycleClass(part)}\`}>{lifecycleLabel(part)}</span> : null}`,
);
...
const receivedOld = `    if (part.status === "received") {
...
const receivedNew = `    if (part.status === "received") {
