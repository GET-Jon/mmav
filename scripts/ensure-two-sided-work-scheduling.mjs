...
changed = patchFile("components/partner/partner-work-list-v4.tsx", (source) => {
  ...
  const suggestedSelectionOld = '{suggestions[work.id]?.length ? <div className="mt-2 flex flex-wrap gap-2">{suggestions[work.id].map((slot) => <button key={slot.startAt} onClick={() => selectSuggestion(work, slot)} className={`rounded-lg border px-3 py-2 text-xs font-black ${schedule.startAt === localInputValue(slot.startAt) ? "border-blue-700 bg-blue-50 text-blue-800" : "border-slate-200 bg-white text-slate-700"}`}>{shortSlot(slot.startAt)}</button>)}</div> : null}<div className="mt-3 text-[10px] font-black uppercase text-slate-400">Or choose another time</div>';
  const suggestedSelectionNew = '{suggestions[work.id]?.length ? <div className="mt-2 flex flex-wrap gap-2">{suggestions[work.id].map((slot) => <button key={slot.startAt} onClick={() => selectSuggestion(work, slot)} className={`rounded-lg border px-3 py-2 text-xs font-black ${schedule.startAt === localInputValue(slot.startAt) ? "border-blue-700 bg-blue-50 text-blue-800" : "border-slate-200 bg-white text-slate-700"}`}>{shortSlot(slot.startAt)}</button>)}</div> : null}{schedule.startAt && suggestions[work.id]?.some((slot) => schedule.startAt === localInputValue(slot.startAt)) ? <div className="mt-3 flex flex-col gap-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-xs font-black text-blue-950">Selected: {dateTime(new Date(schedule.startAt).toISOString())}</div><div className="mt-0.5 text-[11px] font-semibold text-blue-800">This time is selected, but it has not been sent yet.</div></div><button type="button" disabled={workingId === work.id} onClick={() => void saveScheduleValues(work, schedule.startAt, schedule.endAt, "Work time proposed to the Owner for confirmation.")} className="shrink-0 rounded-lg bg-blue-700 px-4 py-2 text-xs font-black text-white disabled:opacity-40">Propose selected time</button></div> : suggestions[work.id]?.length ? <div className="mt-2 text-xs font-semibold text-slate-500">Select a suggested time above, then submit it.</div> : null}<div className="mt-4 text-[10px] font-black uppercase text-slate-400">Or enter a different time</div>';
  if (updated.includes(suggestedSelectionOld)) updated = updated.replace(suggestedSelectionOld, suggestedSelectionNew);

  updated = updated.replace('>Save time</button>', '>{awaitingOwnerSchedule || !hasRequestedSchedule ? "Propose custom time" : "Save custom time"}</button>');
  return updated;
}) || changed;

console.log(changed ? "Enabled two-sided Owner / Partner work-time proposals with explicit suggested-time submission." : "Two-sided work scheduling already applied.");