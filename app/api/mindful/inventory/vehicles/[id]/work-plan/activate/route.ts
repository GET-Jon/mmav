...
function nextBusinessMorning(from = new Date()) {
  ...
  while (true) {
    ...
    if (parts.weekday !== "Sat" && parts.weekday !== "Sun") {
      return localToUtc(parts.year, parts.month, parts.day, 9, 0);
    }
...
export async function POST(
  ...
  try {
    ...
    // Build a first-pass, single-car schedule immediately after authorization.
    // The owner can move these slots later; manual moves replace schedule_source.
    const [{ data: workRows, error: workError }, { data: planItems, error: itemError }] = await Promise.all([
      access.supabase
        .from("mindful_inventory_work_orders")
        .select("id,plan_item_id,estimated_elapsed_minutes,estimated_duration_minutes,assigned_partner_id,status")
    ...
    for (const item of planItems || []) {
      ...
      const durationMinutes = Math.max(
        60,
        Number(work.estimated_elapsed_minutes ?? work.estimated_duration_minutes ?? 60) || 60,
      );

      // If this partner is already committed elsewhere, advance to the first slot
      // after the conflicting assignment. This is deliberately conservative.
      if (partnerId) {
        for (let guard = 0; guard < 20; guard += 1) {
          ...
          cursor = nextBusinessSlot(new Date(conflicts[0].scheduled_end_at));
        }
      }
    ...
    if (suggestedCount > 0) {
      await access.supabase.from("mindful_inventory_history").insert({
        ...
        summary: "Suggested execution schedule created when Work Plan was activated.",
        metadata: { suggestedWorkOrders: suggestedCount, timeZone: COMPANY_TIME_ZONE },
      });
    }
