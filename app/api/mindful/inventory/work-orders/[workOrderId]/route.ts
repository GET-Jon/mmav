...
export async function PATCH(request: Request, context: { params: Promise<{ workOrderId: string }> }) {
  try {
    ...
    const { data: existing, error: existingError } = await access.supabase
      ...
      .select("id,vehicle_id,status,actual_start_at,assigned_partner_id,assigned_user_id,location_id,resource_id,scheduled_start_at,scheduled_end_at,parts_review_status,partner_estimate_status")
      .eq("id", workOrderId)
      .single();
    ...
    if (requestedStatus === "in_progress") {
      ...
      if (!existing.assigned_partner_id && !existing.assigned_user_id) {
        return NextResponse.json({ error: "Assign a performer before work can start." }, { status: 409 });
      }
      if (!existing.location_id) return NextResponse.json({ error: "Choose a work location before work can start." }, { status: 409 });
      if (!existing.scheduled_start_at) return NextResponse.json({ error: "Schedule the Work Order before starting it." }, { status: 409 });
      if (existing.assigned_partner_id && !["approved", "not_required"].includes(existing.partner_estimate_status || "")) {
        return NextResponse.json({ error: "The partner labor estimate must be approved before work can start." }, { status: 409 });
      }
    ...
    const effectiveHasPerformer = Object.prototype.hasOwnProperty.call(patch, "assigned_partner_id") || Object.prototype.hasOwnProperty.call(patch, "assigned_user_id")
      ? Boolean(patch.assigned_partner_id || patch.assigned_user_id)
      : Boolean(existing.assigned_partner_id || existing.assigned_user_id);
    ...
    const setupChanged = performerKey !== undefined || locationProvided || resourceProvided;
    if (setupChanged && existing.scheduled_start_at && !["in_progress", "complete", "cancelled"].includes(requestedStatus || existing.status)) {
      patch.scheduled_start_at = null;
      patch.scheduled_end_at = null;
      patch.proposed_start_at = null;
      patch.proposed_end_at = null;
      patch.partner_confirmation_status = null;
      patch.schedule_source = null;
      if (!requestedStatus && existing.status === "scheduled") patch.status = "ready_to_schedule";
      metadata.scheduleCleared = true;
    }
    ...
    const { data: updated, error: updateError } = await access.supabase
      ...
      .update(patch)
      .eq("id", workOrderId)
      .select("id,status,vehicle_id,assigned_partner_id,assigned_user_id,location_id,resource_id,actual_start_at,actual_end_at,scheduled_start_at")
      .single();
    ...
    if (setupChanged) {
      await access.supabase.from("mindful_inventory_history").insert({
        ...
        summary: metadata.scheduleCleared ? "Work Order setup changed; previous schedule cleared for revalidation." : "Work Order execution assignment updated.",
        metadata: {
    ...
    if (requestedStatus === "complete" || requestedStatus === "cancelled") {
      ...
      if ((count || 0) === 0) {
        await access.supabase
          .from("mindful_inventory_vehicles")
          .update({ phase: "final_qc", next_action: "Complete Final QC", next_action_owner_user_id: access.userId, updated_by: access.userId, updated_at: new Date().toISOString() })
          .eq("id", existing.vehicle_id)
          .eq("company_id", access.company.companyId);
      }
    }
    ...
    return NextResponse.json({
      ...
      scheduleCleared: metadata.scheduleCleared === true,
    });
  } catch (error) {
