...
export default async function PartnerProfilePage({ searchParams }: Props) {
  ...
  return <main className="min-h-screen bg-[#f5f7fb] text-slate-950">
    <header className="border-b border-slate-200 bg-white">
      <div className="mx-auto flex w-full max-w-[1100px] items-center justify-between gap-4 px-4 py-4 sm:px-5 lg:px-7">
        ...
        <div className="flex items-center gap-2">
          {!onboarding ? <nav className="flex flex-wrap gap-1.5"><Link href="/partner/work" className="rounded-lg px-3 py-2 text-sm font-black text-slate-600">Today</Link><Link href="/partner/work?view=all" className="rounded-lg px-3 py-2 text-sm font-black text-slate-600">My Work</Link><Link href="/partner/work#schedule" className="rounded-lg px-3 py-2 text-sm font-black text-slate-600">Schedule</Link>{access.partner.mechanicalInspectionEligible ? <Link href="/partner/inspections" className="rounded-lg px-3 py-2 text-sm font-black text-slate-600">Inspections</Link> : null}<Link href="/partner/profile" className="rounded-lg bg-slate-950 px-3 py-2 text-sm font-black text-white">Profile</Link></nav> : null}
    ...
    <div className="mx-auto w-full max-w-[1100px] px-4 py-6 sm:px-5 lg:px-7">
      <div className="mb-5"><h1 className="text-[30px] font-black tracking-[-0.035em]">{onboarding ? "Confirm your partner profile" : "Your partner profile"}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{onboarding ? "Confirm the details we use to assign and schedule work. You can change these anytime later." : "Keep your contact information, availability, and capabilities current."}</p></div>
