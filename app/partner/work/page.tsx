...
export default async function PartnerWorkPage() {
  ...
  const [workItems, detailingItems, inspectionItems] = await Promise.all([
    getPartnerAssignedWork(access),
    getPartnerDetailingAssignments(access),
    access.partner.mechanicalInspectionEligible ? getPartnerInspectionAssignments(access) : Promise.resolve([]),
  ]);
