...
export default async function PartnerWorkPage({ searchParams }: Props) {
  ...
  const detailedView = rawView === "all";
  ...
  return <main className="min-h-screen bg-[#f5f7fb] text-slate-950">
    <header className="border-b border-slate-200 bg-white">
      <div className="mx-auto flex w-full max-w-[1240px] items-center justify-between gap-4 px-4 py-4 sm:px-5 lg:px-7">
        <div><div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Lot Logic Partner Portal</div><div className="mt-0.5 text-lg font-black">{detailedView ? "My Work" : "Today"}</div></div>
        <div className="flex items-center gap-4">
          <nav className="flex flex-wrap gap-1.5">
            <Link href="/partner/work" className={`rounded-lg px-3 py-2 text-sm font-black ${!detailedView ? "bg-slate-950 text-white" : "text-slate-600"}`}>Today</Link>
            <Link href="/partner/work?view=all" className={`rounded-lg px-3 py-2 text-sm font-black ${detailedView ? "bg-slate-950 text-white" : "text-slate-600"}`}>My Work</Link>
            <Link href="/partner/work#schedule" className="rounded-lg px-3 py-2 text-sm font-black text-slate-600">Schedule</Link>
            {access.partner.mechanicalInspectionEligible ? <Link href="/partner/inspections" className="relative rounded-lg px-3 py-2 text-sm font-black text-slate-600">Inspections{inspectionAttention > 0 ? <span className="ml-1.5 rounded-full bg-blue-600 px-1.5 py-0.5 text-[9px] font-black text-white">{inspectionAttention}</span> : null}</Link> : null}
            <Link href="/partner/profile" className="rounded-lg px-3 py-2 text-sm font-black text-slate-600">Profile</Link>
          </nav>
          <div className="hidden text-right sm:block"><div className="text-sm font-black">{access.partner.name}</div><div className="text-xs text-slate-400">{access.partner.companyName || access.userEmail || "Partner"}</div></div>
          <form action="/api/partner/logout" method="post"><button type="submit" className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-500 hover:text-slate-950">Log out</button></form>
        </div>
      </div>
    </header>
    ...
    <div className="mx-auto w-full max-w-[1240px] px-4 py-6 sm:px-5 lg:px-7">
      ...
      {!detailedView ? <PartnerTodayDashboard partnerName={access.partner.name} workItems={workItems} inspectionItems={inspectionItems} /> : <>
        <div className="mb-4">
          <Link href="/partner/work" className="inline-flex items-center rounded-lg bg-blue-600 px-4 py-2 text-sm font-black text-white shadow-sm hover:bg-blue-700">← Today</Link>
        </div>
