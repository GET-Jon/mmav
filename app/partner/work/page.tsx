...
export default async function PartnerWorkPage() {
  ...
  return <main className="min-h-screen bg-[#f5f7fb] text-slate-950">
    <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex w-full max-w-[1240px] items-center justify-between gap-4 px-4 py-4 sm:px-5 lg:px-7">
      ...
      <div className="flex items-center gap-4"><nav className="flex gap-2">{access.partner.mechanicalInspectionEligible ? <Link href="/partner/inspections" className="rounded-lg px-3 py-2 text-sm font-black text-slate-600">Inspections</Link> : null}<Link href="/partner/work" className="rounded-lg bg-slate-950 px-3 py-2 text-sm font-black text-white">My Work</Link><Link href="/partner/profile" className="rounded-lg px-3 py-2 text-sm font-black text-slate-600">Profile</Link></nav><div className="text-right"><div className="text-sm font-black">{access.partner.name}</div><div className="text-xs text-slate-400">{access.partner.companyName || access.userEmail || "Partner"}</div></div></div>
    </div></header>
