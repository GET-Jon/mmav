...
export function InventoryPartSuggestionsV4({ vehicleId, suggestions, parts }: {
  ...
  async function patchStatus(part: InventoryPartView, status: "received" | "backordered", key?: string) {
    const workingKey = `status:${part.id}:${status}`;
    setWorking(workingKey); setMessage("");
    try {
      const response = await fetch(`/api/mindful/inventory/vehicles/${vehicleId}/parts`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      setMessage(`${part.description}: ${status === "received" ? "received" : "marked delayed — enter a revised ETA."}`);
      if (status === "backordered" && key) {
  ...
  function renderOrderForm(args: { key: string; part?: InventoryPartView | null; workOrderId: string; description: string; searchQuery: string }) {
    ...
    return <div className="mt-3 border-t border-slate-200 pt-3">
      ...
      {args.part?.status === "backordered" ? <div className="mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800">Delayed — enter the revised expected arrival below.</div> : null}
      ...
      <div className="mt-3 flex justify-end"><button type="button" disabled={working !== null} onClick={() => void saveOrder(args)} className="rounded-lg bg-slate-950 px-4 py-2 text-xs font-black text-white disabled:opacity-50">{working === args.key ? "Saving…" : args.part ? (args.part.status === "backordered" ? "Save Revised ETA" : "Save Order Changes") : "Save Order"}</button></div>
    </div>;
  }

  function renderTrackedActions(part: InventoryPartView, key: string) {
    if (part.status === "ordered") {
      return <div className="flex flex-wrap items-center gap-1.5"><button type="button" disabled={working !== null} onClick={() => void patchStatus(part, "received", key)} className="rounded-md bg-emerald-600 px-3 py-1.5 text-[10px] font-black text-white disabled:opacity-50">Mark Received</button><button type="button" disabled={working !== null} onClick={() => void patchStatus(part, "backordered", key)} className="rounded-md border border-amber-200 bg-amber-50 px-3 py-1.5 text-[10px] font-black text-amber-800 disabled:opacity-50">Mark Delayed</button><button type="button" onClick={() => openOrder(key, part)} className="rounded-md bg-slate-950 px-3 py-1.5 text-[10px] font-black text-white">{orderKey === key ? "Close" : "Edit"}</button></div>;
    }
    if (part.status === "backordered") {
      return <div className="flex flex-wrap items-center gap-1.5"><span className="rounded-full border border-amber-300 bg-amber-50 px-2.5 py-1 text-[9px] font-black text-amber-800">Delayed</span><button type="button" disabled={working !== null} onClick={() => void patchStatus(part, "received", key)} className="rounded-md bg-emerald-600 px-3 py-1.5 text-[10px] font-black text-white disabled:opacity-50">Mark Received</button><button type="button" onClick={() => openOrder(key, part)} className="rounded-md bg-slate-950 px-3 py-1.5 text-[10px] font-black text-white">{orderKey === key ? "Close" : "Edit"}</button></div>;
    }
    ...
    if (part.status === "installed") {
      return <div className="flex flex-wrap items-center gap-1.5"><span className="rounded-full border-2 border-emerald-600 bg-emerald-50 px-2.5 py-1 text-[9px] font-black text-emerald-800">✓ Installed</span><button type="button" onClick={() => openOrder(key, part)} className="rounded-md border border-slate-200 px-3 py-1.5 text-[10px] font-black text-slate-600">Edit</button></div>;
    }
    return null;
  }
