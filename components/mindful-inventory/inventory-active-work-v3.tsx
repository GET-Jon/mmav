          ...
          return <article key={work.id} className={`relative ml-4 py-5 pr-5 sm:ml-6 sm:pr-6 ${index ? "border-t border-slate-200" : ""} ${done ? "bg-slate-50/70" : ""}`}>
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_170px] xl:items-start">
              <div className="min-w-0">
                ...
                <div className={`mt-3 rounded-xl border p-3 ${setup.length || execution.length ? "border-amber-200 bg-amber-50/60" : "border-slate-200 bg-slate-50"}`}><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0 text-xs"><div className="text-[9px] font-black uppercase tracking-[0.08em] text-slate-400">Setup</div><div className="mt-1 font-black">{work.scheduledStartAt ? dateTimeLabel(work.scheduledStartAt) : work.proposedStartAt ? `Suggested: ${dateTimeLabel(work.proposedStartAt)}` : "Time not selected"}</div><div className="mt-1 text-slate-600">{work.performerName || "No performer assigned"} · {work.locationName || "No location selected"}{work.resourceName ? ` · ${work.resourceName}` : ""}</div></div>{!done && !showSetupControls ? <button onClick={() => setEditingSetupId(work.id)} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-black">Edit Setup</button> : null}</div>{setup.length ? <div className="mt-3 border-t border-amber-200 pt-2 text-xs font-black text-amber-800">Action: {setup[0]}</div> : null}{!setup.length && execution.length ? <div className="mt-3 border-t border-amber-200 pt-2 text-xs font-black text-violet-800">Before start: {execution[0]}</div> : null}{!setup.length && !execution.length && !done ? <div className="mt-3 border-t border-slate-200 pt-2 text-xs font-black text-emerald-700">✓ Setup resolved · Ready for execution</div> : null}</div>
