...
type ResolutionValue = (typeof resolutionChoices)[number]["value"];

function mergeSuggestions(current: PartSearchSuggestion[], incoming: PartSearchSuggestion[]) {
  const currentById = new Map(current.map((item) => [item.workOrderId, item]));
  return incoming.map((next) => {
    const previous = currentById.get(next.workOrderId);
    if (!previous) return next;
    const previousRecommendations = previous.recommendedParts || [];
    const nextRecommendations = next.recommendedParts || [];
    return {
      ...previous,
      ...next,
      recommendedParts:
        nextRecommendations.length >= previousRecommendations.length
          ? nextRecommendations
...
export function InventoryPartSuggestionsV2({
  vehicleId,
  suggestions,
  parts,
}: {
  ...
  const [expandedWorkOrders, setExpandedWorkOrders] = useState<Set<string>>(() => {
    const attention = suggestions.find((s) =>
      parts.some((p) => p.workOrderId === s.workOrderId && p.status !== "cancelled" && !["received", "installed"].includes(p.status)),
    );
    return new Set(attention ? [attention.workOrderId] : suggestions[0] ? [suggestions[0].workOrderId] : []);
  });

  useEffect(() => {
    setDisplaySuggestions((current) => mergeSuggestions(current, suggestions));
  }, [suggestions]);
      ...
      try {
        ...
        if (!cancelled && response.ok && data.items) {
          ...
          setDisplaySuggestions((current) =>
            current.map((s) => {
              const normalized = byId.get(s.workOrderId);
              if (!normalized) return s;
              const recommendations = normalized.recommendedParts || [];
              const searchQuery = normalized.searchQuery || s.searchQuery;
              return {
                ...s,
                partName: normalized.partName || s.partName,
                searchQuery,
                alternateQueries: normalized.alternateQueries?.length ? normalized.alternateQueries : s.alternateQueries,
                recommendedParts: recommendations.length ? recommendations : s.recommendedParts,
  ...
  async function resolvePart(part: InventoryPartView, resolution: ResolutionValue) {
    const key = `resolve:${part.id}:${resolution}`;
    setWorkingId(key);
    setMessage("");
    try {
      const response = await fetch(`/api/mindful/inventory/vehicles/${vehicleId}/parts/resolve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ partId: part.id, resolution }),
      });
      const data = (await response.json()) as { error?: string };
      if (!response.ok) throw new Error(data.error || "Failed to resolve part source.");
      const label = resolutionChoices.find((item) => item.value === resolution)?.label || labelize(resolution);
      setMessage(`${part.description}: ${label}.`);
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Failed to resolve part source.");
    } finally {
      setWorkingId(null);
    }
  }

  function renderResolution(part: InventoryPartView) {
    return (
      <div className="mt-3 border-t border-slate-100 pt-3">
        <div className="text-[9px] font-black uppercase tracking-[0.09em] text-slate-400">Resolve source</div>
        <div className="mt-2 flex flex-wrap gap-1.5">
          {resolutionChoices.map((choice) => {
            const key = `resolve:${part.id}:${choice.value}`;
            return (
              <button
                key={choice.value}
                type="button"
                disabled={workingId !== null}
                onClick={() => void resolvePart(part, choice.value)}
                className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[10px] font-black text-slate-700 hover:border-slate-400 disabled:opacity-50"
              >
                {workingId === key ? "Saving…" : choice.label}
              </button>
            );
          })}
        </div>
  ...
  function renderEditor({
    ...
    const set = (patch: Partial<Draft>) =>
      setDrafts((current) => ({
        ...current,
        [partKey]: { ...(current[partKey] || draftFromPart(part)), ...patch },
      }));
    ...
    return (
      <div className="mt-3 border-t border-slate-200 pt-3">
        ...
        {part ? renderResolution(part) : null}
        <div className="mt-3 flex justify-end"><button type="button" disabled={workingId !== null} onClick={() => void savePart(workOrderId, description, searchQuery, partKey, part)} className="rounded-lg bg-slate-950 px-4 py-2 text-xs font-black text-white disabled:opacity-50">{workingId === partKey ? "Saving…" : part ? "Save Changes" : "Save & Track"}</button></div>
      </div>
    );
  }
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div><div className="text-xs font-black uppercase tracking-[0.1em] text-slate-400">Parts by Work Order</div><h2 className="mt-1 text-xl font-black">Parts & sourcing</h2><p className="mt-1 text-sm font-medium text-slate-500">Each Work Order contains its suggested and tracked parts. Add, edit, source, and update arrival details in place.</p></div>
          ...
          return (
            <article key={suggestion.workOrderId} className="overflow-hidden rounded-2xl border border-slate-200">
              <button type="button" onClick={() => toggleWorkOrder(suggestion.workOrderId)} className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left hover:bg-slate-50">
                <div><div className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Work Order</div><div className="mt-1 text-lg font-black">{suggestion.workOrderTitle}</div><div className="mt-2 flex flex-wrap gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${confidenceClass(suggestion.confidence)}`}>{confidenceLabel(suggestion.confidence)}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black text-slate-600">{suggestion.recommendedParts.length} suggested</span><span className="rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-black text-blue-700">{workParts.length} tracked</span>{attention ? <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-black text-amber-800">{attention} need attention</span> : null}</div></div>
                ...
                <div className="border-t border-slate-100 bg-slate-50/50 px-5 py-4">
                  <div className="space-y-2">
                      ...
                      return (
                        <div key={partKey} className="rounded-xl border border-slate-200 bg-white px-4 py-3">
                          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                            <div><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-black">{rec.name}</span><span className={`rounded-full px-2 py-0.5 text-[9px] font-black ${recommendationClass(rec.need)}`}>{recommendationLabel(rec.need)}</span>{existing ? <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[9px] font-black text-blue-700">{labelize(existing.status)}</span> : null}</div><div className="mt-1.5 text-[11px] font-semibold text-slate-500">{existing ? `${existing.supplier || "Source not entered"} · Expected ${shortDate(existing.etaAt)} · ${money(existing.quotedUnitPrice)}` : rec.searchQuery}</div></div>
                            <div className="flex flex-wrap gap-1.5">{sources.map((source) => <a key={source.key} href={source.url} target="_blank" rel="noreferrer" className="rounded-md border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-black text-slate-600">{source.key === "turn14" ? "Turn 14" : source.label} ↗</a>)}<button type="button" onClick={() => openEditor(partKey, existing)} className="rounded-md bg-slate-950 px-3 py-1.5 text-[11px] font-black text-white">{editing ? "Close" : existing ? "Edit" : "+ Add"}</button></div>
                          </div>
                          {existing && !editing ? renderResolution(existing) : null}
                    ...
                    {unmatchedTracked.map((part) => {
                      ...
                      return (
                        <div key={part.id} className="rounded-xl border border-slate-200 bg-white px-4 py-3">
                          <div className="flex items-center justify-between gap-3"><div><div className="flex items-center gap-2"><span className="text-sm font-black">{part.description}</span><span className="rounded-full bg-blue-50 px-2 py-0.5 text-[9px] font-black text-blue-700">{labelize(part.status)}</span></div><div className="mt-1.5 text-[11px] font-semibold text-slate-500">{part.supplier || "Source not entered"} · Expected {shortDate(part.etaAt)} · {money(part.quotedUnitPrice)}</div></div><button type="button" onClick={() => openEditor(partKey, part)} className="rounded-md bg-slate-950 px-3 py-1.5 text-[11px] font-black text-white">{editing ? "Close" : "Edit"}</button></div>
                          {!editing ? renderResolution(part) : null}
                    ...
                    <div className="rounded-xl border border-dashed border-slate-300 bg-white px-4 py-3">
                      ...
                      <div className="mt-2 flex gap-2"><input value={otherNames[suggestion.workOrderId] || ""} onChange={(e) => setOtherNames((current) => ({ ...current, [suggestion.workOrderId]: e.target.value }))} placeholder="Add a part Lot Logic didn't suggest" className="min-w-0 flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm" /><button type="button" disabled={!(otherNames[suggestion.workOrderId] || "").trim()} onClick={() => { const key = `${suggestion.workOrderId}:other`; setDrafts((current) => ({ ...current, [key]: current[key] || draftFromPart(null) })); setExpandedPartKey(key); }} className="rounded-lg bg-slate-950 px-4 py-2 text-xs font-black text-white disabled:bg-slate-200">+ Add</button></div>
                      {expandedPartKey === `${suggestion.workOrderId}:other` && (otherNames[suggestion.workOrderId] || "").trim() ? renderEditor({ partKey: `${suggestion.workOrderId}:other`, workOrderId: suggestion.workOrderId, description: (otherNames[suggestion.workOrderId] || "").trim(), searchQuery: `${fitment} ${(otherNames[suggestion.workOrderId] || "").trim()}` }) : null}
                    </div>
                  </div>
                </div>
              ) : null}
