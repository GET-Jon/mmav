...
type AvailabilityState = "available" | "outside_hours" | "conflict";
...
type Props = {
  ...
  afterAssignPath?: string;
};

export function MechanicalInspectorAssignment({ vehicleId, options, inspection, pendingFindingReviews = 0, pendingUpgradeReviews = 0, afterAssignPath }: Props) {
  const router = useRouter();
  const initialStart = inspection?.requestedStartAt?.slice(0, 16) || "";
  const [requestedStartAt, setRequestedStartAt] = useState(initialStart);
  const recommended = useMemo(() => {
    const available = requestedStartAt ? options.filter((option) => availableAt(option, requestedStartAt)) : options;
    return available[0] || options[0] || null;
  ...
  const [customTimeOpen, setCustomTimeOpen] = useState(Boolean(initialStart));
  ...
  async function assign() {
    ...
    setMessage("");
    try {
      const response = await fetch(`/api/mindful/inventory/vehicles/${vehicleId}/inspection-assignment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ partnerId, requestedStartAt: requestedStartAt ? new Date(requestedStartAt).toISOString() : null, inspectionFee: fee }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Inspection could not be assigned.");
      setMessage("Inspection assigned. It is now waiting for the mechanic to confirm.");
      if (afterAssignPath) {
        router.push(afterAssignPath);
        router.refresh();
      } else {
        router.refresh();
      }
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Inspection could not be assigned.");
    } finally {
      setWorking(false);
    }
  }
  ...
  async function review(decision: "accept" | "revision") {
    ...
    try {
      const response = await fetch(`/api/mindful/inventory/vehicles/${vehicleId}/inspection-assignment`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ decision, notes: revisionNotes }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Inspection review could not be saved.");
      setMessage(decision === "accept" ? "Inspection accepted. Work Plan can now be built." : "Revision request sent to the inspector.");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Inspection review could not be saved.");
    } finally {
      setWorking(false);
    }
  }
  ...
  if (inspection?.status === "submitted") return <section className="rounded-2xl border border-blue-200 bg-blue-50 p-5 shadow-sm">
    <div className="text-xs font-black uppercase tracking-[0.1em] text-blue-600">Owner Review Required</div>
    <h2 className="mt-1 text-xl font-black text-slate-950">Mechanical inspection submitted</h2>
    <p className="mt-1 text-sm text-slate-600">Review the mechanic&apos;s findings and requested upgrades below. Accepting the inspection completes Mechanical; it does not assign resulting work to the inspector.</p>
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      <button disabled={working || reviewRemaining > 0} onClick={() => void review("accept")} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-black text-white disabled:bg-slate-300">Accept Inspection</button>
      <button disabled={working || !revisionNotes.trim()} onClick={() => void review("revision")} className="rounded-xl border border-amber-300 bg-white px-4 py-2.5 text-sm font-black text-amber-800">Request Full Revision</button>
    </div>
    {message ? <div className="mt-3 text-sm font-bold text-slate-700">{message}</div> : null}
  </section>;
  ...
  if (inspection?.performedByPartnerId && !["complete", "cancelled"].includes(inspection.status)) {
    ...
    return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="text-xs font-black uppercase tracking-[0.1em] text-slate-400">Mechanical Inspector</div>
          <h2 className="mt-1 text-xl font-black">{assigned?.name || "Assigned mechanic"}</h2>
          <div className="mt-1 text-sm text-slate-500">{inspection.status.replaceAll("_", " ")} · {when(inspection.scheduledStartAt || inspection.requestedStartAt)} · {money(inspection.inspectionFee)}</div>
        </div>
        <span className="rounded-full bg-blue-50 px-3 py-1.5 text-xs font-black uppercase text-blue-700">{inspection.status.replaceAll("_", " ")}</span>
      </div>
      {message ? <div className="mt-3 text-sm font-bold text-slate-700">{message}</div> : null}
    </section>;
  }
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <h2 className="text-xl font-black">Assign a mechanical inspector</h2>
      {recommended ? <span className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700">Recommended: {recommended.name}</span> : null}
    </div>
    ...
    {options.length ? <>
      ...
      {customTimeOpen ? <div className="mt-3 inline-flex max-w-full flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
        <span className="text-xs font-black uppercase tracking-[0.08em] text-slate-500">Custom time</span>
        ...
        {selected && requestedStartAt ? (() => { const state = availabilityState(selected, requestedStartAt); return <span className={`text-xs font-bold ${state === "available" ? "text-emerald-700" : "text-red-700"}`}>{state === "available" ? "Available" : state === "outside_hours" ? "Outside standard hours" : "Schedule conflict"}</span>; })() : null}
      </div> : null}
