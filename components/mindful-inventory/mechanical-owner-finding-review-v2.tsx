...
function partPriceLabel(part: InventoryFindingView["mechanicalSuggestedParts"][number]) {
  ...
  if (low !== null || high !== null) {
    ...
    return {
      value:
        resolvedLow !== null && resolvedHigh !== null && Math.abs(resolvedHigh - resolvedLow) > 0.009
...
export function MechanicalOwnerFindingReviewV2({
  ...
  const [alternatePartners, setAlternatePartners] = useState<Record<string, string>>(() =>
    Object.fromEntries(findings.map((finding) => [finding.id, finding.ownerPreferredPartnerId || ""])),
  );
  ...
  const inspector = partnerOptions.find((partner) => partner.id === inspectorPartnerId) || null;
  ...
  async function review(
    ...
    if (decision === "accept" && !cost.pricingComplete) {
      setOpenNotes((current) => ({ ...current, [finding.id]: true }));
      setMessage("Complete the labor and required part pricing before approving this repair and its spend.");
      return;
    }
    ...
    if (
      ...
      setMessage(`Choose who should handle ${finding.title} before approving and routing the repair.`);
      return;
    }
    ...
    return (
      <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.06em] text-slate-400">Mechanic</div>
            <div className="mt-1 text-sm font-black text-slate-900">{mechanicName}</div>
            {inspector?.secondaryLabel ? (
              <div className="text-xs font-semibold text-slate-500">{inspector.secondaryLabel}</div>
            ) : null}
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.06em] text-slate-400">Recommended action</div>
            <div className="mt-1 text-sm font-black text-slate-900">
              {finding.mechanicalRecommendedAction || "No recommendation entered"}
            </div>
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.06em] text-slate-400">Can perform</div>
            <div className={`mt-1 text-sm font-black ${needsDifferentPartner ? "text-amber-800" : "text-slate-900"}`}>
              {finding.mechanicalCanPerform === null ? "Unknown" : finding.mechanicalCanPerform ? "Yes" : "No"}
            </div>
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.06em] text-slate-400">Labor</div>
            <div className="mt-1 text-sm font-black text-slate-900">
              {finding.mechanicalLaborHours === null ? "TBD" : `${finding.mechanicalLaborHours} hr`}
            </div>
          </div>
        </div>
  ...
  function renderApprovalSummary(finding: InventoryFindingView) {
    ...
    return (
      <section className={`rounded-xl border-2 p-4 ${cost.pricingComplete ? "border-emerald-200 bg-emerald-50/50" : "border-amber-300 bg-amber-50/60"}`}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className={`text-[10px] font-black uppercase tracking-[0.09em] ${cost.pricingComplete ? "text-emerald-700" : "text-amber-800"}`}>
              Approval summary
            </div>
            <div className="mt-2 flex flex-wrap gap-x-6 gap-y-2 text-sm">
              ...
              <span>
                <span className="font-semibold text-slate-500">Parts estimate </span>
          ...
          <div className="sm:text-right">
            ...
            <div className={`mt-1 text-2xl font-black ${cost.pricingComplete ? "text-slate-950" : "text-amber-900"}`}>
              {approvalAuthorizationLabel(cost)}
            </div>
          </div>
        </div>
        <div className={`mt-3 text-xs font-semibold leading-5 ${cost.pricingComplete ? "text-emerald-900" : "text-amber-900"}`}>
          {cost.pricingComplete ? (
            <>
              Approving authorizes this repair, authorizes spend up to {money(cost.totalHigh)}, and includes the repair in the Work Plan.
              {cost.usesAiPartEstimate ? " One or more part prices are AI estimates; spending above this authorization requires additional approval." : ""}
            </>
          ) : (
            <>
              Pricing is incomplete. Request clarification so labor and every required part have a usable price before this repair can be approved.
            </>
          )}
        </div>
      </section>
  ...
  function renderParts(finding: InventoryFindingView) {
    ...
    return (
      <section>
        <div className="text-[10px] font-black uppercase tracking-[0.09em] text-slate-400">Parts supporting this approval</div>
        <div className="mt-2 divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white">
          {finding.mechanicalSuggestedParts.map((part, index) => {
            ...
            return (
              <div key={`${part.description}-${index}`} className="flex flex-col gap-2 px-3 py-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="shrink-0 sm:text-right">
                  ...
                  <div className={`text-[10px] font-bold uppercase tracking-[0.06em] ${price.source === "Price needed" ? "text-amber-700" : "text-slate-400"}`}>
                    {price.source}
                  </div>
                </div>
              </div>
  ...
  function renderPartnerRouting(finding: InventoryFindingView) {
    ...
    if (!needsDifferentPartner) {
      if (!finding.mechanicalCanPerform) return null;
      return (
    ...
    return (
      <div className={`rounded-xl border p-3 ${selectedAlternatePartner ? "border-emerald-200 bg-emerald-50/70" : "border-amber-200 bg-amber-50/70"}`}>
        <div className="flex items-center justify-between gap-3">
          <div className={`text-[10px] font-black uppercase tracking-[0.07em] ${selectedAlternatePartner ? "text-emerald-700" : "text-amber-800"}`}>
            {selectedAlternatePartner ? "Partner selected" : "Alternate partner required"}
          </div>
          {!selectedAlternatePartner ? <span className="text-[10px] font-bold text-amber-700">Required</span> : null}
        </div>
        <select
          value={selectedAlternatePartner}
          onChange={(event) => setAlternatePartners((current) => ({ ...current, [finding.id]: event.target.value }))}
          className={`mt-2 w-full rounded-lg border bg-white px-3 py-2 text-sm font-bold text-slate-800 ${selectedAlternatePartner ? "border-emerald-300" : "border-amber-300"}`}
        >
          <option value="">Choose partner</option>
          {availablePartners.map((partner) => (
            <option key={partner.id} value={partner.id}>
              {partner.displayName}{partner.secondaryLabel ? ` · ${partner.secondaryLabel}` : ""}
            </option>
          ))}
        </select>
        <div className={`mt-1.5 text-[10px] font-semibold leading-4 ${selectedAlternatePartner ? "text-emerald-700" : "text-amber-800"}`}>
          {selectedAlternatePartner
            ? "This Partner will carry into the Work Plan and Active Work."
            : `${inspector?.displayName || "The mechanic"} said they cannot perform this repair.`}
        </div>
      </div>
    );
  }
  ...
  function renderUnresolvedFinding(finding: InventoryFindingView) {
    ...
    return (
      <article key={finding.id} className={`rounded-2xl border bg-white p-4 shadow-sm sm:p-5 ${clarification ? "border-amber-300" : "border-slate-200"}`}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <h4 className="text-lg font-black leading-6 text-slate-950">{finding.title}</h4>
            {finding.description ? <p className="mt-1 text-sm leading-5 text-slate-600">{finding.description}</p> : null}
          ...
          <span className={`shrink-0 self-start rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.06em] ${clarification ? "bg-amber-100 text-amber-800" : "bg-blue-50 text-blue-700"}`}>
        ...
        <div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="min-w-0 flex-1">
              {noteOpen ? (
                <label className="block">
                  <span className="text-[10px] font-black uppercase tracking-[0.08em] text-slate-400">Owner note / question</span>
                  <input
                    value={notes[finding.id] || ""}
                    onChange={(event) => setNotes((current) => ({ ...current, [finding.id]: event.target.value }))}
                    className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-800"
                    placeholder="Ask the mechanic about scope, price, parts, or recommendation"
                  />
                </label>
              ) : (
                <button
          ...
          <div className="flex shrink-0 flex-wrap gap-2 border-t border-slate-100 pt-3 lg:border-l lg:border-t-0 lg:pl-3 lg:pt-0">
            <button
              ...
              title={!cost.pricingComplete ? "Complete pricing before approval" : needsDifferentPartner && !selectedAlternatePartner ? "Choose a Partner before approval" : undefined}
            >
              {needsDifferentPartner
                ? `Approve & Route · up to ${money(cost.totalHigh)}`
                : cost.hasRange
                  ? `Approve Repair · up to ${money(cost.totalHigh)}`
                  : `Approve Repair · ${money(cost.totalHigh)}`}
            </button>
            <button
              disabled={workingId === finding.id}
  ...
  function renderResolvedFinding(finding: InventoryFindingView) {
    ...
    const selectedPartnerId = finding.ownerPreferredPartnerId || (finding.mechanicalCanPerform ? inspectorPartnerId : null);
    const selectedPartner = partnerOptions.find((partner) => partner.id === selectedPartnerId) || null;

    return (
      <article key={finding.id} className="rounded-xl border border-slate-200 bg-slate-50/70 px-4 py-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <div className="flex items-start gap-2">
              <span className={`mt-0.5 font-black ${accepted ? "text-emerald-600" : "text-slate-400"}`}>{accepted ? "✓" : "—"}</span>
              <div className="min-w-0">
                <div className="font-black text-slate-900">{finding.title}</div>
                <div className="mt-0.5 text-xs font-semibold text-slate-500">
                  {accepted
                    ? `Approved · ${approvalAuthorizationLabel(cost)}${selectedPartner ? ` · ${selectedPartner.displayName}` : ""}`
                    : "Dismissed"}
                </div>
              </div>
            </div>
          ...
          <button
            type="button"
            onClick={() => setOpenResolved((current) => ({ ...current, [finding.id]: !expanded }))}
        ...
        {expanded ? (
          <div className="mt-3 space-y-3 border-t border-slate-200 pt-3">
            {finding.description ? <p className="text-sm text-slate-600">{finding.description}</p> : null}
            {renderAssessment(finding)}
            ...
            {accepted && selectedPartner ? (
              <div className="text-xs font-semibold text-slate-600">
                ...
                {selectedPartner.displayName}{selectedPartner.secondaryLabel ? ` · ${selectedPartner.secondaryLabel}` : ""}
              </div>
            ) : null}
            {finding.mechanicalOwnerReviewNotes ? (
              <div className="text-xs font-semibold text-slate-600">
  ...
  return (
    <div className="mt-4 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4">
        <div>
          ...
          <div className="mt-1 text-xs font-semibold text-slate-500">
            Approve the repair, its authorized spend, and its Partner before it enters the Work Plan.
          </div>
        </div>
        <span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.08em] ${unresolvedFindings.length ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-700"}`}>
      ...
      {unresolvedFindings.length ? (
        <section>
          <div className="mb-3 flex items-center justify-between gap-3">
            <h3 className="text-xs font-black uppercase tracking-[0.1em] text-slate-500">Needs your decision</h3>
            <span className="text-xs font-bold text-slate-400">{unresolvedFindings.length}</span>
          </div>
          <div className="space-y-3">{unresolvedFindings.map(renderUnresolvedFinding)}</div>
        </section>
      ) : null}
      ...
      {resolvedFindings.length ? (
        <section>
          <div className="mb-3 flex items-center justify-between gap-3">
            <h3 className="text-xs font-black uppercase tracking-[0.1em] text-slate-500">Resolved</h3>
            <span className="text-xs font-bold text-slate-400">{resolvedFindings.length}</span>
          </div>
      ...
      {message ? (
        <div className="rounded-lg bg-slate-50 px-3 py-2 text-sm font-bold text-slate-700">{message}</div>
      ) : null}
    </div>
  );
}
