      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm font-medium text-slate-500">Hands-on labor measures technician capacity. Turnaround measures how long the job occupies the workflow/calendar. They are intentionally separate.</p>
            <Link href="/mindful/inventory/schedule" className="mt-4 inline-flex rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-black text-slate-700 hover:border-slate-400">Open cross-vehicle Schedule →</Link>
          </div>
          <div className="grid min-w-[400px] grid-cols-4 gap-2">
            ...
            <div className="rounded-xl bg-blue-50 px-3 py-3"><div className="text-[10px] font-black uppercase text-blue-500">Labor Load</div><div className="mt-1 text-lg font-black text-blue-800">{hours(totalLabor)}</div></div>
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${work.status === "complete" ? "bg-emerald-100 text-emerald-700" : work.status === "in_progress" ? "bg-blue-100 text-blue-700" : work.status === "scheduled" ? "bg-violet-100 text-violet-700" : "bg-slate-100 text-slate-700"}`}>{labelize(work.status)}</span>
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-slate-500">
                    <span className="rounded-full bg-slate-50 px-3 py-1.5">Category: {labelize(work.category)}</span>
                    <span className="rounded-full bg-slate-50 px-3 py-1.5">Budget: {money(work.approvedBudget)}</span>
                    <span className="rounded-full bg-blue-50 px-3 py-1.5 text-blue-700">Hands-on labor: {hours(work.estimatedLaborMinutes)}</span>
                    <span className="rounded-full bg-violet-50 px-3 py-1.5 text-violet-700">Turnaround: {hours(turnaround)}{legacyOnly ? " · legacy AI estimate" : ""}</span>
                    {work.scheduledStartAt ? <span className="rounded-full bg-emerald-50 px-3 py-1.5 text-emerald-700">Scheduled: {new Date(work.scheduledStartAt).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</span> : null}
                  </div>
                </div>
