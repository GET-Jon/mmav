...
type InventoryVehicleDetailDrawerProps = {
...
export function InventoryVehicleDetailDrawer({
  vehicle,
  onClose,
}: InventoryVehicleDetailDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/30" role="dialog" aria-modal="true">
      ...
      <aside className="relative z-10 h-full w-full max-w-2xl overflow-y-auto border-l border-slate-200 bg-slate-50 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-5 sm:p-6">
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <h3 className="text-sm font-black text-slate-950">Operational State</h3>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              ...
              <label>
                ...
                <select className={inputClass} value={grade} onChange={(e) => setGrade(e.target.value as InventoryVehicleGrade | "")}>
                  ...
                  {(["a", "b", "c", "d", "e"] as InventoryVehicleGrade[]).map((value) => (
              ...
              <label>
                ...
                <select className={inputClass} value={priority} onChange={(e) => setPriority(e.target.value as InventoryVehiclePriority)}>
                  {(["1", "2", "3"] as InventoryVehiclePriority[]).map((value) => (
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <h3 className="text-sm font-black text-slate-950">Next Action & Readiness</h3>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <label className="sm:col-span-2">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h3 className="text-sm font-black text-slate-950">Hold Overlay</h3>
                <p className="mt-1 text-xs leading-5 text-slate-500">A hold pauses normal progress without changing the vehicle phase.</p>
              </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <h3 className="text-sm font-black text-slate-950">Reference</h3>
          ...
          <div className="sticky bottom-0 -mx-5 border-t border-slate-200 bg-slate-50/95 px-5 py-4 backdrop-blur sm:-mx-6 sm:px-6">
            <div className="flex items-center justify-between gap-4">
              <div className={`text-sm font-bold ${status && status !== "Saved." ? "text-red-600" : "text-emerald-700"}`}>
                {status}
              </div>
              <button
                type="submit"
                disabled={saving}
                className="rounded-xl bg-slate-950 px-5 py-2.5 text-sm font-black text-white disabled:bg-slate-300"
              >
