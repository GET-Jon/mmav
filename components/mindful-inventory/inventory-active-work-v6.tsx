...
function estimateIssue(work: InventoryWorkOrderView) {
  ...
  if (work.partnerEstimateStatus === "awaiting_estimate") return "Waiting for partner labor quote";
  if (work.partnerEstimateStatus === "awaiting_review") return "Partner labor quote needs Owner approval";
  if (work.partnerEstimateStatus === "revision_requested") return "Waiting for revised partner labor quote";
  return null;
}

function quoteDone(work: InventoryWorkOrderView) {
  if (!work.assignedPartnerId) return true;
  return ["approved", "not_required"].includes(work.partnerEstimateStatus || "");
}

function quoteDetail(work: InventoryWorkOrderView) {
  if (!work.assignedPartnerId) return "Not required";
...
function stateFor(work: InventoryWorkOrderView) {
  ...
  if (estimateIssue(work)) return { label: "Quote Pending", cls: "bg-violet-100 text-violet-800" };
...
export function InventoryActiveWork({ vehicleId, vehicle: _vehicle, workOrders, performerOptions, locationOptions, resourceOptions, parts, partSuggestions }: {
  ...
  async function confirmNoParts(work: InventoryWorkOrderView) {
    ...
    try {
      ...
      setRowMessages((current) => ({ ...current, [work.id]: { type: "success", text: "No parts required. Next: choose the Partner." } }));
      router.refresh();
    } catch (error) {
          ...
          <div className="min-w-0 flex-1">
            ...
            {statusFocus ? <>
              ...
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-sm font-semibold text-slate-600">
                ...
                <span>{statusFocus.performerName || "Partner not assigned"}</span>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="ml-4 border-l border-slate-200 sm:ml-6">
          {workOrders.map((work, index) => {
            ...
            const estimate = estimateIssue(work);
            const quoteComplete = quoteDone(work);
            ...
            return (
              <article key={work.id} className={`relative ml-4 py-5 pr-5 sm:ml-6 sm:pr-6 ${index ? "border-t border-slate-200" : ""} ${done ? "bg-slate-50/70" : ""}`}>
                ...
                <div className="grid gap-4 xl:grid-cols-[minmax(260px,0.85fr)_minmax(470px,1.45fr)_150px] xl:items-start">
                  ...
                  <div className="min-w-0">
                    ...
                    {!done && (issue || editing) ? <div className="mt-3 rounded-xl border border-blue-200 bg-blue-50/40 p-3">
                      ...
                      {(partnerActive || editing) && work.partsReviewComplete ? <div className="mt-2 rounded-lg border border-slate-200 bg-white p-3"><div className="mb-2 flex items-center justify-between gap-2"><div className="text-[10px] font-black uppercase text-slate-400">2 · Partner</div>{suggestion ? <button disabled={workingId === work.id} onClick={() => void patchWork(work.id, { performerKey: suggestion.key }, `Assigned ${suggestion.displayName}.`)} className="rounded-full bg-blue-100 px-2 py-1 text-[9px] font-black text-blue-800">Suggest {suggestion.displayName}</button> : null}</div><select disabled={workingId === work.id} value={performerKey(work)} onChange={(event) => void patchWork(work.id, { performerKey: event.target.value }, "Partner updated.")} className="w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs font-bold"><option value="unassigned">Choose Partner</option>{performerOptions.map((option) => <option key={option.key} value={option.key}>{option.displayName}</option>)}</select></div> : null}

                      {(quoteActive || editing) && work.partsReviewComplete && work.performerName ? <div className="mt-2 rounded-lg border border-slate-200 bg-white p-3"><div className="mb-1 text-[10px] font-black uppercase text-slate-400">3 · Quote</div>{!work.assignedPartnerId ? <div className="text-xs font-bold text-emerald-700">✓ Internal Mindful work · quote not required.</div> : work.partnerEstimateStatus === "awaiting_review" ? <div className="flex flex-wrap items-center justify-between gap-3"><div className="text-xs font-bold text-violet-800">{work.performerName}'s labor quote is ready for Owner review.</div><a href="#partner-estimate-review" className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white">Review Quote →</a></div> : work.partnerEstimateStatus === "revision_requested" ? <div className="text-xs font-bold text-violet-800">Revision requested. Waiting for {work.performerName} to submit a revised labor quote.</div> : quoteComplete ? <div className="text-xs font-bold text-emerald-700">✓ Labor quote approved.</div> : <div className="text-xs font-bold text-violet-800">Waiting for {work.performerName} to submit a blind labor quote.</div>}</div> : null}

                      {(locationActive || editing) && work.partsReviewComplete && work.performerName && quoteComplete ? <div className="mt-2 rounded-lg border border-slate-200 bg-white p-3"><div className="mb-2 text-[10px] font-black uppercase text-slate-400">4 · Location</div><div className="grid gap-2 sm:grid-cols-2"><select disabled={workingId === work.id} value={work.locationId || ""} onChange={(event) => void patchWork(work.id, { locationId: event.target.value || null }, "Location updated.")} className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs font-bold"><option value="">Choose location</option>{locationOptions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select><select disabled={workingId === work.id || !work.locationId} value={work.resourceId || ""} onChange={(event) => void patchWork(work.id, { resourceId: event.target.value || null }, "Resource updated.")} className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs font-bold"><option value="">No specific resource</option>{resources.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select></div></div> : null}

                      {(scheduleActive || editing) && work.partsReviewComplete && work.performerName && quoteComplete && work.locationId ? <div className="mt-2 rounded-lg border border-slate-200 bg-white p-3"><div className="mb-2 text-[10px] font-black uppercase text-slate-400">5 · Schedule</div>{!work.partsReadyForExecution ? <div className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800">Locked until all required parts are received. {partsPendingLabel(work)}</div> : <div className="flex flex-col gap-2 sm:flex-row"><input disabled={workingId === work.id} type="datetime-local" value={draftValue} onChange={(event) => setScheduleDrafts((current) => ({ ...current, [work.id]: event.target.value }))} className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs font-bold" /><button disabled={workingId === work.id || !draftValue} onClick={() => void scheduleWork(work, draftValue)} className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white disabled:bg-slate-200 disabled:text-slate-400">{work.scheduledStartAt ? "Save New Time" : "Save Schedule"}</button></div>}</div> : null}
                    </div> : null}
