...
type RecommendationDraft = { recommendedAction: string; partsRequired: string; laborHours: string; canPerform: "" | "yes" | "no" };
type NewFindingDraft = RecommendationDraft & { title: string; description: string };

const emptyRecommendation: RecommendationDraft = { recommendedAction: "", partsRequired: "", laborHours: "", canPerform: "" };
...
export function PartnerInspectionList({ items, typicalDurationHours }: { items: PartnerInspectionItem[]; typicalDurationHours: number | null }) {
  ...
  function recommendationFor(finding: PartnerInspectionItem["findings"][number]): RecommendationDraft {
    return recommendations[finding.id] || {
      recommendedAction: finding.recommendedAction || "",
      partsRequired: finding.partsRequired || "",
  ...
  async function submitFinding(item: PartnerInspectionItem, finding: PartnerInspectionItem["findings"][number]) {
    ...
    const saved = await act(item, {
      ...
      partsRequired: recommendation.partsRequired,
  ...
  return <div className="space-y-3">{items.map((item) => {
    ...
    return <section key={item.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      {open ? <div className="border-t border-slate-100 px-5 py-5">
        ...
        {editable ? <>
          <div className="mt-5">
            <div className="text-xs font-black uppercase tracking-[0.1em] text-slate-400">Findings to validate</div>
            <div className="mt-3 space-y-3">{item.findings.map((finding) => {
              const recommendation = recommendationFor(finding);
              ...
              return <div key={finding.id} className={`rounded-xl border ${reviewed ? "border-emerald-200 bg-emerald-50/20" : "border-slate-200"}`}>
                <div className="flex flex-wrap items-start justify-between gap-3 p-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">{validationChoices.map((choice) => {
                      const selected = selectedStatus === choice.value;
                      return <button key={choice.value} type="button" disabled={working === item.id} onClick={() => {
                        setSelectedStatuses((current) => ({ ...current, [finding.id]: choice.value }));
                        setExpandedFindings((current) => ({ ...current, [item.id]: finding.id }));
                      }} className={`rounded-lg border px-3 py-2 text-xs font-black ${selected ? "border-slate-950 bg-slate-950 text-white" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}>{choice.label}</button>;
                    })}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {reviewed ? <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-black uppercase text-emerald-800">Reviewed ✓</span> : <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black uppercase text-slate-500">Pending</span>}
                    {reviewed ? <span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${dispositionTone(finding.validationStatus)}`}>{statusLabel(finding.validationStatus)}</span> : null}
                    <button type="button" onClick={() => setExpandedFindings((current) => ({ ...current, [item.id]: expanded ? null : finding.id }))} className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[10px] font-black text-slate-600">{expanded ? "Collapse" : "Open"}</button>
                  </div>
                </div>

                {expanded ? <div className="border-t border-slate-100 p-4">
                  <textarea value={findingNotes[finding.id] ?? finding.validationNotes ?? ""} onChange={(e) => setFindingNotes((current) => ({ ...current, [finding.id]: e.target.value }))} placeholder="Diagnostic notes" className="min-h-16 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" />
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    <input value={recommendation.recommendedAction} onChange={(e) => setRecommendations((current) => ({ ...current, [finding.id]: { ...recommendation, recommendedAction: e.target.value } }))} placeholder="Recommended action (e.g. replace both front lower arms)" className="rounded-xl border border-slate-200 px-3 py-2 text-sm" />
                    <input inputMode="decimal" value={recommendation.laborHours} onChange={(e) => setRecommendations((current) => ({ ...current, [finding.id]: { ...recommendation, laborHours: e.target.value } }))} placeholder="Labor hours" className="rounded-xl border border-slate-200 px-3 py-2 text-sm" />
                    <input value={recommendation.partsRequired} onChange={(e) => setRecommendations((current) => ({ ...current, [finding.id]: { ...recommendation, partsRequired: e.target.value } }))} placeholder="Parts required" className="rounded-xl border border-slate-200 px-3 py-2 text-sm sm:col-span-2" />
                    <select value={recommendation.canPerform} onChange={(e) => setRecommendations((current) => ({ ...current, [finding.id]: { ...recommendation, canPerform: e.target.value as RecommendationDraft["canPerform"] } }))} className="rounded-xl border border-slate-200 px-3 py-2 text-sm"><option value="">Can you perform this work?</option><option value="yes">Yes — I can perform it</option><option value="no">No — another specialist needed</option></select>
                    <button type="button" disabled={working === item.id || !selectedStatus} onClick={() => void submitFinding(item, finding)} className="rounded-xl bg-slate-950 px-4 py-2 text-sm font-black text-white disabled:bg-slate-200 disabled:text-slate-400">Submit Finding</button>
                  </div>
                </div> : null}
              </div>;
            })}</div>
          ...
          <div className="mt-5 rounded-xl border border-dashed border-slate-300 p-4"><div className="text-xs font-black uppercase tracking-[0.1em] text-slate-400">New mechanical finding</div><div className="mt-2 grid gap-2 sm:grid-cols-2"><input value={fresh.title} onChange={(e) => setNewFinding((current) => ({ ...current, [item.id]: { ...fresh, title: e.target.value } }))} placeholder="Finding title" className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold" /><input value={fresh.description} onChange={(e) => setNewFinding((current) => ({ ...current, [item.id]: { ...fresh, description: e.target.value } }))} placeholder="What did you find?" className="rounded-xl border border-slate-200 px-3 py-2 text-sm" /><input value={fresh.recommendedAction} onChange={(e) => setNewFinding((current) => ({ ...current, [item.id]: { ...fresh, recommendedAction: e.target.value } }))} placeholder="Recommended action" className="rounded-xl border border-slate-200 px-3 py-2 text-sm" /><input inputMode="decimal" value={fresh.laborHours} onChange={(e) => setNewFinding((current) => ({ ...current, [item.id]: { ...fresh, laborHours: e.target.value } }))} placeholder="Labor hours" className="rounded-xl border border-slate-200 px-3 py-2 text-sm" /><input value={fresh.partsRequired} onChange={(e) => setNewFinding((current) => ({ ...current, [item.id]: { ...fresh, partsRequired: e.target.value } }))} placeholder="Parts required" className="rounded-xl border border-slate-200 px-3 py-2 text-sm sm:col-span-2" /><select value={fresh.canPerform} onChange={(e) => setNewFinding((current) => ({ ...current, [item.id]: { ...fresh, canPerform: e.target.value as RecommendationDraft["canPerform"] } }))} className="rounded-xl border border-slate-200 px-3 py-2 text-sm"><option value="">Can you perform this work?</option><option value="yes">Yes — I can perform it</option><option value="no">No — another specialist needed</option></select></div><button disabled={working === item.id || !fresh.title.trim()} onClick={() => void act(item, { action: "add_finding", title: fresh.title, description: fresh.description, recommendedAction: fresh.recommendedAction, partsRequired: fresh.partsRequired, laborHours: fresh.laborHours, canPerform: fresh.canPerform === "" ? null : fresh.canPerform === "yes" })} className="mt-2 rounded-lg border border-slate-300 px-3 py-2 text-xs font-black">Add Finding</button></div>
