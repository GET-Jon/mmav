  ...
  return <div className="space-y-3">{items.map((item) => {
    ...
    return <section key={item.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      {open ? <div className="border-t border-slate-100 px-5 py-5">
        ...
        {editable ? <>
          <div className="mt-5"><div className="text-xs font-black uppercase tracking-[0.1em] text-slate-400">Findings to validate</div><div className="mt-3 space-y-3">{item.findings.map((finding) => { const recommendation = recommendationFor(finding); return <div key={finding.id} className="rounded-xl border border-slate-200 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="font-black">{finding.title}</div>{finding.description ? <div className="mt-1 text-sm text-slate-600">{finding.description}</div> : null}</div><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black uppercase text-slate-500">{finding.validationStatus.replaceAll("_", " ")}</span></div>
            <textarea value={findingNotes[finding.id] ?? finding.validationNotes ?? ""} onChange={(e) => setFindingNotes((current) => ({ ...current, [finding.id]: e.target.value }))} placeholder="Diagnostic notes" className="mt-3 min-h-16 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" />
            <div className="mt-3 grid gap-2 sm:grid-cols-2"><input value={recommendation.recommendedAction} onChange={(e) => setRecommendations((current) => ({ ...current, [finding.id]: { ...recommendation, recommendedAction: e.target.value } }))} placeholder="Recommended action (e.g. replace both front lower arms)" className="rounded-xl border border-slate-200 px-3 py-2 text-sm" /><input inputMode="decimal" value={recommendation.laborHours} onChange={(e) => setRecommendations((current) => ({ ...current, [finding.id]: { ...recommendation, laborHours: e.target.value } }))} placeholder="Labor hours" className="rounded-xl border border-slate-200 px-3 py-2 text-sm" /><input value={recommendation.partsRequired} onChange={(e) => setRecommendations((current) => ({ ...current, [finding.id]: { ...recommendation, partsRequired: e.target.value } }))} placeholder="Parts required" className="rounded-xl border border-slate-200 px-3 py-2 text-sm sm:col-span-2" /><select value={recommendation.canPerform} onChange={(e) => setRecommendations((current) => ({ ...current, [finding.id]: { ...recommendation, canPerform: e.target.value as RecommendationDraft["canPerform"] } }))} className="rounded-xl border border-slate-200 px-3 py-2 text-sm"><option value="">Can you perform this work?</option><option value="yes">Yes — I can perform it</option><option value="no">No — another specialist needed</option></select></div>
            <div className="mt-3 flex flex-wrap gap-2">{[["confirmed","Confirm"],["not_found","Not Found"],["changed","Changed"],["needs_diagnosis","Needs Diagnosis"]].map(([status,label]) => <button key={status} disabled={working === item.id} onClick={() => void act(item, { action: "validate_finding", findingId: finding.id, status, notes: findingNotes[finding.id] ?? finding.validationNotes ?? "", recommendedAction: recommendation.recommendedAction, partsRequired: recommendation.partsRequired, laborHours: recommendation.laborHours, canPerform: recommendation.canPerform === "" ? null : recommendation.canPerform === "yes" })} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50">{label}</button>)}</div></div>; })}</div></div>
